...
public class PlantBreathAttackProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity.getPersistentData().getDouble("IA") == 0) {
			if (entity instanceof BuffaloEntity) {
				((BuffaloEntity) entity).setAnimation("breath");
			}
		...
		if (entity.getPersistentData().getDouble("IA") > 27 && entity.getPersistentData().getDouble("IA") < 52) {
			for (int index0 = 0; index0 < (int) entity.getPersistentData().getDouble("BreathRange"); index0++) {
				for (int index1 = 0; index1 < (int) particles; index1++) {
					...
					if (world instanceof ServerLevel _level)
						_level.sendParticles(ParticleTypes.ENCHANTED_HIT, XPar, YPar, ZPar, 5, (0.075 + Range * 0.05), (0.075 + Range * 0.05), (0.075 + Range * 0.05), 0);
					{
						...
						for (Entity entityiterator : _entfound) {
							if (!(entityiterator == entity)) {
								...
								if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
									_entity.addEffect(new MobEffectInstance(MobEffects.POISON, 120, 3));
							}
						}
					}
					loop = loop + 1;

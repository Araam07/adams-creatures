...
public class FenrirBiteAttackProcedure {
		...
		if (entity.getPersistentData().getDouble("IA") == 0) {
			...
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("adams_creatures:wolfbite")), SoundSource.HOSTILE, 1, 1);
				} else {
					_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("adams_creatures:wolfbite")), SoundSource.HOSTILE, 1, 1, false);
				}
			}
			...
			for (int index0 = 0; index0 < 4; index0++) {
					...
					_level.sendParticles(ParticleTypes.ENCHANTED_HIT, XPar, YPar, ZPar, 5, 0.25, 0.25, 0.25, 0);
				{
					...
					for (Entity entityiterator : _entfound) {
						if (!(Items.SHIELD == (entityiterator instanceof LivingEntity _entUseItem14 ? _entUseItem14.getUseItem() : ItemStack.EMPTY).getItem()) && !(entityiterator == entity)
